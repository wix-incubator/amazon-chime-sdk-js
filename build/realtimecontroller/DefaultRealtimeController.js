...
var DefaultRealtimeController = /** @class */ (function () {
    ...
    DefaultRealtimeController.prototype.realtimeUnsubscribeFromVolumeIndicator = function (attendeeId, callback) {
        var _this = this;
        this.wrap(function () {
            var attendeeCallbacks = _this.state.volumeIndicatorCallbacks[attendeeId] || [];
            var callbackIndex = attendeeCallbacks.indexOf(callback);
            if (callbackIndex > -1) {
                console.log('Before remove: ', _this.state.volumeIndicatorCallbacks[attendeeId].length);
                attendeeCallbacks.splice(callbackIndex, 1);
                console.log('After remove: ', _this.state.volumeIndicatorCallbacks[attendeeId].length);
            }
        });
    };
